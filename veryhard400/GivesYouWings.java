...
public class GivesYouWings {
    ...
    public int solve(String[] maze) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++) {
                if (maze[i].charAt(j) == 'S')
                    s = new State(i, j, (1 << 8) - 1);
                else if (maze[i].charAt(j) == 'T')
                    t = new State(i, j, (1 << 8) - 1);
        ...
        while (!q.isEmpty()) {
            ...
            // Integer.toBinaryString(curr.mask) + " " + dp[curr.x][curr.y][curr.mask]);
            ...
            for (int move = 0; move < 4; move++) {
                ...
                if (nx >= 0 && nx < n && ny >= 0 && ny < m && maze[nx].charAt(ny) != '#') {
                    // System.out.println("Prethodnik e "+ curr.x + " " + curr.y + " sl e " + nx + "
                    // " + ny);
