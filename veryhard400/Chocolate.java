...
public class Main{
    ...
    static long rekruzija(int r, int c, int b, int n){
        ...
        if (n == 0 && r*c >= b) return  1;
        else if (n == 0 && (r*c) < b ) return 0;
        else if (n>0 && (r==0 || c==0)) return 0;
 
        long x = 0, y = 0;
 
        for (int i=1; i<c; i++) if (i*r >= b) {
            x += rekruzija(r,c-i,b,n-1);
            x %= 1000000007;
        }
 
        for (int i=1; i<r; i++) if (i*c >= b) {
